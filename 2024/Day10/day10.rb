...
matrix = lines.map { |line| line.chomp.chars.map(&:to_i) }
counts = Array.new(matrix.size) { Array.new(matrix[0].size) { Set.new } }
...
def debug(array_of_sets)
  array_of_sets.map { |row| row.map { |cell| cell.size } }
end

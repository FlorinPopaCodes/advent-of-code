...
def count_corners(perimeter_edges, edges_map, field_map)
  perimeter_lines = Hash.new { |h, k| h[k] = [] }

  perimeter_edges.each do |edge|
    y, x = edge

    perimeter_lines["x#{x+0.9}"] << y+1 if x == 0
    perimeter_lines["x#{x+2.1}"] << y+1 if x == edges_map[0].size - 1
    perimeter_lines["y#{y+0.9}"] << x+1 if y == 0
    perimeter_lines["y#{y+2.1}"] << x+1 if y == edges_map.size - 1


    perimeter_lines["x#{x+0.9}"] << y+1 if x > 0 && field_map[y][x-1] != field_map[y][x]
    perimeter_lines["x#{x+2.1}"] << y+1 if x < edges_map[0].size - 1 && field_map[y][x+1] != field_map[y][x]
    perimeter_lines["y#{y+0.9}"] << x+1 if y > 0 && field_map[y-1][x] != field_map[y][x]
    perimeter_lines["y#{y+2.1}"] << x+1 if y < edges_map.size - 1 && field_map[y+1][x] != field_map[y][x]
  end

  perimeter_lines.each do |line, points|
    perimeter_lines[line] = points.slice_when {|i, j| i+1 != j }.count
  end

  # Bug. I should be calculating the unique sides, not the number of lines
  perimeter_lines.sum { |_, count| count }
end

groups.each do |_, group|
  group[:perimeter] = count_corners(group[:edges], edges, field)
end
